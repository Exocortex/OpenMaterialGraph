...
var clampColor = function( color ) {
  return {
    r: Math.max( Math.min( 1.0, color.r ), 0.0 ),
    g: Math.max( Math.min( 1.0, color.g ), 0.0 ),
    b: Math.max( Math.min( 1.0, color.b ), 0.0 )
  };
}

var getColorFromInput = function( input, isFalloff, optionalNodePath ) {
  var result = null;
  if( input ) {
    //console.log( 'getBaseColorFromInput: input', input.path() );
    if( input.isConnected() ) {
      result = findColorInNode(input.value.node, isFalloff, optionalNodePath);
    }
...
var findColorInNode = function( node, isFalloff, optionalNodePath ) {
  ...
  switch( node.spec.name ) {
    ...
    case "TexCombineColor":
      var multiplier = node.inputs['texture_multiplier'].value;
      if (multiplier === undefined) multiplier = 1.0;

      var color = node.inputs['color'];
      var texture = node.inputs['texture'];

      if (texture && texture.isConnected()) {
        // it's a valid texture, let get its color
        texture = getColorFromInput(texture, isFalloff, optionalNodePath);
        resultColor.r = texture.r * multiplier;
        resultColor.g = texture.g * multiplier;
        resultColor.b = texture.b * multiplier;

        if (color && multiplier < 1) {
          // interpolate
          color = color.value;
          var inv = 1.0 - multiplier;
          resultColor.r += color.r * inv;
          resultColor.g += color.g * inv;
          resultColor.b += color.b * inv;
        }
      }
      else {
        if (color) {
    ...
    case "TexMix":
      ...
      if( input && input.isConnected() ) {
        color = getColorFromInput(input, false, optionalNodePath);
        if( color ) {
        //console.log( "TexNoiseMax/TexMix color1: ", color );
          resultColor.r += color.r * 0.5;
          resultColor.g += color.g * 0.5;
          resultColor.b += color.b * 0.5;
        }
      }
      input = node.inputs.color2;
      if( input && input.isConnected() ) {
        color = getColorFromInput(input, false, optionalNodePath);
        if( color ) {
        //console.log( "TexNoiseMax/TexMix color2: ", color );
          resultColor.r += color.r * 0.5;
          resultColor.g += color.g * 0.5;
          resultColor.b += color.b * 0.5;
        }
    ...
    case "TexFalloff":
      ...
      if( input && input.isConnected() ) {
        var color = getColorFromInput(input, false, optionalNodePath);
        if( color ) {
        //console.log( "TexFalloff color: ", color );
          resultColor.r = color.r;
          resultColor.g = color.g;
          resultColor.b = color.b;
        }
...
/**
 * Multiple cases for 'map' and 'color' properties that need to be matched together
 */
function mapColorProperties(properties, propMap, propColor, propSrc, node, isFalloff) {
  properties[propMap] = getImageFromInput(node.inputs[propSrc], isFalloff);
  if (properties[propMap]) {
    properties[propColor] = { r:1, g:1, b:1 };
  }
  else {
    properties[propColor] = clampColor(getColorFromInput(node.inputs[propSrc], isFalloff));
  }
};
...
var getPhysicalPropertiesFromBRDFVRayMtl = function( node ) {
  ...
  mapColorProperties(properties, 'baseMap', 'baseColor', 'diffuse', node, false);

  properties['baseFalloff'] = true;
  mapColorProperties(properties, 'baseFalloffMap', 'baseFalloffColor', 'diffuse', node, true);
  ...
  // try using 'refract' to determine the opacity
  var refractColor = clampColor(getColorFromInput( node.inputs['refract'], false ));
  ...
  // if that doesn't work, use the 'opacity'
  if( ! properties['opacityMap'] ) {
    properties['opacityMap'] = getImageFromInput( node.inputs['opacity'], false );
  }
  else if( properties['opacityFactor'] === 1 ) {
    var opacityColor = clampColor(getColorFromInput( node.inputs['opacity'], false ));
    var opacityMono = ( opacityColor.r + opacityColor.g + opacityColor.b ) * 0.3333333;
    properties['opacityFactor'] = opacityMono;
  }

  var roughness = clampColor(getColorFromInput( node.inputs['reflect_glossiness'], false )).r;
  if (roughness !== undefined) {
    properties['roughness'] = 1.0 - Math.sqrt(roughness);
  }
  ...
  var reflectColor = getColorFromInput(node.inputs['reflect'], false);
  var reflectMap   = getImageFromInput(node.inputs['reflect'], false);
  if (reflectMap) {
    reflectColor.r = 1.0;
    reflectColor.g = 1.0;
    reflectColor.b = 1.0;
  }
  // var reflectIntensity = 0.0;
  var baseColor = properties['baseColor'];
  if (!properties['baseMap'] && baseColor.r === 0.0 && baseColor.g === 0.0 && baseColor.b === 0.0 &&
      (reflectMap || reflectColor.r > 0.0 || reflectColor.g > 0.0 || reflectColor.b > 0.0))
  {
    properties['metallic' ] = 1.0;
    properties['baseColor'] = reflectColor;
    properties['baseMap'  ] = reflectMap;
    reflectMap = null;
  }
  else {
    var reflectIntensity = Math.max(reflectColor.r, reflectColor.g, reflectColor.b) * 0.5;
    properties['specularColor'] = { r: reflectIntensity, g: reflectIntensity, b: reflectIntensity };
    properties['specularMap'  ] = reflectMap;
  }

  // base color / metallic interpolation using fresnel
  // map [1:12.1] --> [0:1]
  // var usingFresnel = node.inputs["fresnel"].value;
  // var fresnel = (getColorFromInput(node.inputs['fresnel_ior'], false).r - 1.0) / 11.1;
  // fresnel = Math.min(1.0, Math.max(0.0, fresnel));

  // set the metallic value
  //properties['metallic']    = reflectIntensity;
  //properties['metallicMap'] = reflectMap;

  //console.log( "physicalProperties", properties );
  return properties;
};
