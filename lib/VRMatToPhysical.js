...
var findColorInNode = function( node, isFalloff, optionalNodePath ) {
  ...
  switch( node.spec.name ) {
    ...
    case "TexCombineColor":
      ...
      resultColor = clampColor(resultColor);
      break;
...
function mapColorProperties(properties, propMap, propColor, propSrc, node, isFalloff) {
  ...
  if (properties[propMap]) {
    ...
    properties[propColor] = getColorFromInput(node.inputs[propSrc], isFalloff);
  }
};
...
var getPhysicalPropertiesFromBRDFVRayMtl = function( node ) {
  ...
  mapColorProperties(properties, 'translucencyMap', 'translucencyColor', 'translucency_color', node, false);
