...
var findBaseBRDFVRayMtl = function(node, optionalNodePath) {
  ...
  console.log(
    'ERROR, can not traverse node in findBaseBRDFVRayMtl: ' +
      node.label +
      ' <' +
      node.spec.name +
      '>'
...
var getColorFromInput = function(input, isFalloff, ignoreTexture) {
  if (input) {
    //console.log( 'getBaseColorFromInput: input', input.path() );
    if (input.isConnected()) {
      return findColor(input.value.node, isFalloff, null, ignoreTexture);
    }
...
var findColor = function(node, isFalloff, optionalNodePath, ignoreTexture) {
  ...
  switch (node.spec.name) {
    case 'TexCombineColor':
      ...
      if (input && input.isConnected()) {
        ...
        if (color) {
          //console.log( "TexCombineColor texture: ", color );
      ...
      if (color && (ignoreTexture || !gotTexture)) {
        //console.log( "TexCombineColor color: ", color );
        resultColor.r += color.r;
        resultColor.g += color.g;
        resultColor.b += color.b;
      }

      //console.log( "TexCombineColor resultColor: ", resultColor );
      return clampColor(resultColor);
...
var getImagePropertiesFromTexBitmap = function(node) {
  if (!node) return null;
  if (node.spec.name !== 'TexBitmap')
    throw new Error('unknown node type: ' + node.spec.name);
  var properties = {};
...
var gamma = 1.0 / 2.2;
var gammaCorrection = function(color) {
  return {
...
var getPhysicalPropertiesFromBRDFVRayMtl = function(node) {
  ...
  if (properties['baseMap']) {
    ...
    properties['baseColor'] = gammaCorrection(
      getColorFromInput(node.inputs['diffuse'], false)
    );
  }

  properties['baseFalloffMap'] = getImageFromInput(
    node.inputs['diffuse'],
    true
  );
  if (properties['baseFalloffMap']) {
    ...
    properties['baseFalloffColor'] = gammaCorrection(
      getColorFromInput(node.inputs['diffuse'], true)
    );
  }
  ...
  if (properties['opacityFactor'] === 1.0) {
    var opacityColor = getColorFromInput(node.inputs['opacity'], false, true); // we want the real factor, even if there's a texture
    var opacityMono =
  ...
  properties['roughness'] = getColorFromInput(
    node.inputs['reflect_glossiness'],
    false
  ).r;
  if (properties['roughness'] !== undefined) {
    properties['roughness'] = Math.sqrt(1 - properties['roughness']);
  }
  properties['roughnessMap'] = getImageFromInput(
    node.inputs['reflect_glossiness'],
    false
  );
  ...
  var reflectColor = gammaCorrection(
    getColorFromInput(node.inputs['reflect'], false)
  );
