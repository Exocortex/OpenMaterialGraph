...
VRMatValueParser.fileName = function( value, param, node ) {
  ...
  var result = {};
  result[param.name] = path.basename( value );
  return result;
};
...
VRMatValueParser.componentTransform = function( value, param, node ) {
  ...
  result[param.name] = {
    rotation: {
      x: parseFloat( base.x_rotation[0] ),
      y: parseFloat( base.y_rotation[0] ),
      z: parseFloat( base.z_rotation[0] )
    },
    translation: {
      y: parseFloat( base.y_offset[0] ),
      x: parseFloat( base.x_offset[0] ),
      z: parseFloat( base.z_offset[0] )
    },
    scale: {
      x: parseFloat( base.x_scale[0] ),
      y: parseFloat( base.y_scale[0] ),
      z: parseFloat( base.z_scale[0] )
    },
    mirror: {
      x: parseFloat( base.x_mirror[0] ),
      y: parseFloat( base.y_mirror[0] ),
      z: parseFloat( base.z_mirror[0] )
    }
  };
  return result;
};
...
VRMatValueParser.vector = function( value, param, node ) {
  return null;
};

VRMatValueParser.transform = function( value, param, node ) {
  return null;
};
...
VRMatValueParser.parsers = [
  ...
  },
  {
    name: "vector",
    match: function( parameter ) {
      return ( parameter.type === 'vector' );
    },
    parsers: [ VRMatValueParser.vector ]
  },
  {
    name: "transform",
    match: function( parameter ) {
      return ( parameter.type === 'transform' );
    },
    parsers: [ VRMatValueParser.transform ]
  }
];
...
VRMatValueParser.parse = function( param, node ) {
  ...
  var parser = _.find( VRMatValueParser.parsers, function( parser ) {
